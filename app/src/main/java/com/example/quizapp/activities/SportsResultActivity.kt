...
class SportsResultActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        tvName.text = intent.getStringExtra(SportsConstants.USER_NAME)

        val totalQuestions = intent.getIntExtra(SportsConstants.TOTAL_QUESTIONS,0)
        val correctQuestions  = intent.getIntExtra(SportsConstants.CORRECT_ANSWERS,0)

        tvScore.text = "Your Score is $correctQuestions out of $totalQuestions"

        btnFinish.setOnClickListener{
            startActivity(Intent(this, SportsMainActivity::class.java))
            finish()
        }
    }
}