...
class HistoryResultActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        tvName.text = intent.getStringExtra(HistoryConstants.USER_NAME)

        val totalQuestions = intent.getIntExtra(HistoryConstants.TOTAL_QUESTIONS,0)
        val correctQuestions  = intent.getIntExtra(HistoryConstants.CORRECT_ANSWERS,0)

        tvScore.text = "Your Score is $correctQuestions out of $totalQuestions"

        btnFinish.setOnClickListener{
            startActivity(Intent(this, HistoryMainActivity::class.java))
            finish()
        }
    }
}